...
class BoardingGroupCorpusTest {
    @Test
    fun corpusSeparatesCheckInSequenceFromDisplayedBoardingGroup() {
        for (sample in SAMPLES) {
            val parsed = IataBcbp.parse(sample.rawMessage)

            assertNotNull(parsed, sample.name)
            parsed!!
            assertEquals(sample.operatingCarrier, parsed.carrierCode, sample.name)
            assertEquals(sample.checkInSequence, parsed.checkInSequence, sample.name)

            assertEquals(sample.airlineUse, parsed.airlineData, sample.name)

            assertNull(standardBoardingGroup(parsed), sample.name)
        }
    }

    private fun standardBoardingGroup(parsed: IataBcbp.Parsed): String? = null

    private data class BoardingGroupSample(
        val name: String,
        val rawMessage: String,
        val operatingCarrier: String,
        val checkInSequence: String?,
        val airlineUse: String?,
        val displayedBoardingGroup: String?,
    )
    ...
    private companion object {
        ...
        private fun minimalBcbp(carrier: String, checkInSequence: String, airlineUse: String): String =
            "M1" +
                fixed("DOE/JOHN", 20) +
                "E" +
                fixed("ABC123", 7) +
